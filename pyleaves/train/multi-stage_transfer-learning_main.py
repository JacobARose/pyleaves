...
#     parser.add_argument('-d', '--dataset_names', default=['PNAS','Fossil'], type=str, nargs=2, help='Requires 2 args identifying datasets by name in order of input to the pipeline. Stage 1: train + validate, Stage 2: finetune + validate + test')
    parser.add_argument('-source', '--source_datasets', default='PNAS+Leaves', type=str, help='1 or more names separated by a "+" identifying datasets by name to combine for the source domain. Stage 1: train + validate')
    parser.add_argument('-target', '--target_datasets', default='Fossil', type=str, help='1 or more names separated by a "+" identifying datasets by name to combine for the target domain. Stage 2: finetune + validate + test')
...
#     tf.set_random_seed(seed_value)
    ...
    import pyleaves
    from pyleaves.utils import ensure_dir_exists, HyperParameters, set_random_seed
    ...
    #########################################
    #Initialize hyperparameter search iterator
    hparam_sampler = HyperParameters().initialize_iterator(args)
    
    def generate_multidataset_run_name(source_dataset='', target_dataset=''):
        return f'source-{source_dataset}_target-{target_dataset}'
    ...
    for num_finished, hparam in enumerate(hparam_sampler):
        ...
        args.model_name = hparam['model_name']
        args.source_datasets = hparam['source_datasets']
        args.target_datasets = hparam['target_datasets']
        args.run_name = generate_multidataset_run_name(source_dataset=hparam['source_datasets'], target_dataset=hparam['target_datasets'])
#         args.run_name = '_'.join(hparam['dataset_names'])
        args.base_learning_rate = hparam['base_learning_rate']
        args.batch_size = hparam['batch_size']

...
def train_val_test_split(image_paths, labels, data_df, test_size=0.3, val_size=0.3, random_seed=2376, verbose=True):

    train_paths, test_paths, train_labels, test_labels  = train_test_split(image_paths, labels, test_size=test_size, random_state=random_seed, shuffle=True, stratify=data_df['label'])
...
#     one_hot_labels = one_hot_encode_labels(data_df['label'].values)
    data_splits = train_val_test_split(image_paths, labels, data_df, val_size=val_size, test_size=test_size, verbose=verbose)

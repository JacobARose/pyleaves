...
def initialize_experiment(PARAMS, experiment_start_time=None):
    if PARAMS['transfer_to_PNAS']:
        PARAMS['experiment_name'] = '_'.join([PARAMS['dataset_name']+'_to_PNAS', PARAMS['model_name']])
    else:
    ...
    PARAMS['tfrecord_dir'] = os.path.join(PARAMS['log_dir'],'tfrecord_dir')
...
#     date_format = '%Y-%m-%d_%H-%M-%S'
    ...
    PARAMS['experiment_name'] = '_'.join([PARAMS['dataset_name'], PARAMS['model_name']])
    PARAMS['experiment_dir'] = os.path.join(PARAMS['neptune_experiment_dir'], PARAMS['experiment_name'])
    ensure_dir_exists(PARAMS['experiment_dir'])
...
def load_data(dataset_name='PNAS',
    ...
    if use_tfrecords:
        ...
        if shuffle_train:
            train_data = train_data.shuffle(int(data_files.num_samples*splits['train']),seed=seed, reshuffle_each_iteration=True)
...
def build_or_restore_model(PARAMS, ):

    ensure_dir_exists(PARAMS['checkpoints_dir'])
    checkpoints = [os.path.join(PARAMS['checkpoints_dir'], name) for name in os.listdir(PARAMS['checkpoints_dir'])]
    if checkpoints:
        latest_checkpoint = max(checkpoints, key=os.path.getctime)
        print(f'found {len(checkpoints)} checkpoints')
        print("Restoring from", latest_checkpoint)
        return tf.keras.models.load_model(latest_checkpoint)
    print("No previously created saved model, creating a new model")
...
def train_pyleaves_dataset(PARAMS):
    ...
    train_dataset, validation_dataset, STAGE1_data_files, excluded = create_dataset(dataset_name=PARAMS['dataset_name'],
    ...
    PARAMS['splits_size'] = {'train':{},
                             'validation':{}}
    PARAMS['splits_size']['train'] = int(STAGE1_data_files.num_samples*PARAMS['splits']['train'])
    PARAMS['splits_size']['validation'] = int(STAGE1_data_files.num_samples*PARAMS['splits']['validation'])
    ...
    if PARAMS['transfer_to_PNAS'] or PARAMS['transfer_to_Fossil']:
        ...
        train_dataset, validation_dataset, STAGE2_data_files, STAGE2_excluded = create_dataset(dataset_name=PARAMS['stage_2']['dataset_name'], #PARAMS['dataset_name'],
                                                                                               threshold=PARAMS['threshold'],
                                                                                               batch_size=PARAMS['BATCH_SIZE'],
                                                                                               buffer_size=PARAMS['buffer_size'],
                                                                                               exclude_classes=PARAMS['exclude_classes'],
        ...
        PARAMS['splits_size'] = {'train':{},
                                 'validation':{}}
        PARAMS['splits_size']['train'] = int(STAGE2_data_files.num_samples*PARAMS['splits']['train'])
        PARAMS['splits_size']['validation'] = int(STAGE2_data_files.num_samples*PARAMS['splits']['validation'])
...
#                  tf.keras.callbacks.ModelCheckpoint(filepath=PARAMS['model_filepattern'], monitor='val_loss', verbose=1, save_best_only=False)
# ]
#     PARAMS['base_learning_rate'] = PARAMS['lr']
#     PARAMS['input_shape'] = (*PARAMS['target_size'],PARAMS['num_channels'])
#
...
if __name__=='__main__':
    ...
    PARAMS = restore_or_initialize_experiment(PARAMS, restore_last=False, prefix='log_dir__', verbose=2)
    # pprint(PARAMS)
    neptune.init(project_qualified_name=PARAMS['neptune_project_name'])
    with neptune.create_experiment(name=PARAMS['experiment_name']+'-'+str(dict(PARAMS['splits'])), params=PARAMS):
        train_pyleaves_dataset(PARAMS)
